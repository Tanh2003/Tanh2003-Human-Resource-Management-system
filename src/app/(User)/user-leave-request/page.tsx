...
const LeaveRequest = () => {
  ...
  const { user } = useUser(); 
  const [employee, setEmployee] = React.useState<any>(""); // Set initial state to null
  ...
  const getLeaveRequestData = async () => {
    try {
     console.log("id cua employee:", employee);
      const response = await handleListIDLeaveRequest(employee);
      if (response.errCode === 0) {
        const LeaveRequest = response.allLeaveRequest.map(
          (LeaveRequest: any, index: number) => ({
            ...
            start_date: new Date(LeaveRequest.start_date).toLocaleDateString(
              "en-GB"
            ),
            end_date: new Date(LeaveRequest.end_date).toLocaleDateString(
              "en-GB"
            ),
  ...
  const getEmployeesIdData = async (userId: string) => {
    try {
      ...
      if (response.errCode === 0) {
        ...
        setEmployee(response.allEmployees._id);
